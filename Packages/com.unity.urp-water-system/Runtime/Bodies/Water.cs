...
namespace WaterSystem
{
    ...
    public class Water : WaterBody // TODO rename to Ocean
    {
        ...
        private void Update()
        {
            ...
            // Override mesh surface transform before generating
            _meshSurface.transformMatrix = Matrix4x4.TRS(
                transform.position,
                transform.rotation,
                Vector3.one
            );
            ...
            {
                _waterMaterial = CoreUtils.CreateEngineMaterial(
                    ProjectSettings.Instance.resources.WaterShader);
                _waterMaterial.enableInstancing = true;
            }

            SetMaterialProperties();

            // Pass scene cam as BOTH cam and debugCam so frustum matches what you see
            _meshSurface.GenerateSurface(ref _meshSettings, ref cam, ref cam, ref _waterMaterial, gameObject.layer);
        }
